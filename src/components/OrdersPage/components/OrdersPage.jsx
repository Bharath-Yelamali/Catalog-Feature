...
// Self-contained fetchOrders function
async function fetchOrders(username, accessToken, search = '', field = 'orderName') {
...
const OrdersPage = ({ username, accessToken }) => {
  ...
  const fetchAllOrders = useCallback(async (search = '', field = searchField) => {
    ...
    if (field === 'createdBy') backendField = 'created_by_id@aras.keyed_name';
    if (field === 'orderName') backendField = 'orderName'; // keep as is for order name
    ...
    setError(null);
    try {
      // Pass mapped backendField to fetchOrders
      const data = await fetchOrders(username, accessToken, search, backendField);
